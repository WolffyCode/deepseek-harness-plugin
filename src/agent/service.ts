...
import { EngineSuiteChildBridge, type EngineSuiteChildBridgeRequest } from '../orchestration/bridge.js'
...
import type { AgentStreamEvent } from './provider-contract.js'
import type { JsonObject, JsonValue } from '../codex/json-rpc.js'
import type { EngineSuiteCommandView } from '../types.js'
import {
  createParentChildLineageStore,
  type ParentChildLineageDescriptor,
  type ParentChildLineageEvent,
  type ParentChildLineageStore,
} from '../orchestration/lineage.js'
...
interface SessionPreparationLike {
...
function sameSelection(a: EngineSelection, b: EngineSelection): boolean {
...
interface ClaudeCommandCatalogSession {
  readonly catalog: { readonly skills: readonly { readonly name: string }[] }
  listCommands(): readonly ClaudeCommandEntry[]
  refreshCatalog(): Promise<{ readonly commands: readonly ClaudeCommandEntry[]; readonly skills?: readonly { readonly name: string }[] }>
}

interface CodexCommandTransport {
...
function commandCatalogRuntime(runtime: import('./runtime.js').ExternalEngineRuntime): CommandCatalogRuntime {
  return runtime as unknown as CommandCatalogRuntime
}
...
function commandString(value: JsonValue | undefined, fallback: string): string {
  return typeof value === 'string' ? value : fallback
}

function commandSource(value: string | undefined): 'command' | 'skill' {
  return value === 'skill' ? 'skill' : 'command'
}

function normalizeCommand(command: ClaudeCommandEntry, skills: ReadonlySet<string>): EngineSuiteCommandView {
  return {
    name: command.name,
...
function turnFailure(session: Session): string | undefined {
  ...
  if (last === undefined || last.type !== 'turn/end') return 'child agent did not finish a turn'
  if (last.data.reason.kind === 'completed') return undefined
  if (last.data.reason.kind === 'error') return last.data.reason.error.message
...
export class EngineSuiteAgentService implements AgentFactory {
  ...
  private readonly children = new Map<string, Set<EngineSuiteAgentHandle>>()
  private readonly bindings: ExternalEngineBindingStore
  private readonly childBridge: EngineSuiteChildBridge
  ...
  async delegate(parentSessionId: string, request: DelegateExternalAgentOptions): Promise<{ readonly handle: EngineSuiteAgentHandle; readonly text: string; readonly lineage: ParentChildLineageDescriptor }> {
    ...
    if (request.task.trim().length === 0) throw new Error('child task must not be empty')
    ...
    try {
      ...
      if (sessions === undefined) throw new Error('Engine Suite requires Harness SessionStore for child delegation')
  ...
  private async delegateNative(
    parent: EngineSuiteAgentHandle,
    request: DelegateExternalAgentOptions,
    selection: EngineSelection,
    depth: number,
    ...
    try {
      ...
      if ((current?.status === 'running' || current?.status === 'detached') && current.terminalStatus === undefined) this.lineageStore.append({ parentSessionId: lineage.parentSessionId, nativeTaskId: lineage.nativeTaskId, childSessionId: lineage.childSessionId, type: 'result', data: text })
  ...
  private async delegateFromBridge(request: EngineSuiteChildBridgeRequest): Promise<{ readonly childSessionId: string; readonly text: string; readonly nativeTaskId: string }> {
  ...
  private defaultSelection(agentOptions: AgentOptions | undefined): EngineSelection {
  ...
  private async createExternalOn(
  ...
  ): Promise<EngineSuiteAgentHandle> {
    ...
    const approval = ownerCtx.get('approval') as { request(request: { agent: unknown; toolName: string; reason?: string }): Promise<'allowed-once' | 'rejected' | 'cancelled' | 'unavailable'> } | undefined
    ...
    let launch: Awaited<ReturnType<EngineSuiteRuntime['openEngine']>>
    try {
    ...
    try {
      ...
      if (existingHeader === undefined || !sessionHasConversation(session)) {
        ...
        if (profile.reasoningEffort !== undefined) {
          config.reasoningEffort = profile.reasoningEffort as NonNullable<LlmCallConfig['reasoningEffort']>
        }
      ...
      const observeRuntime = (runtime: import('./runtime.js').ExternalEngineRuntime, runtimeProvider = provider.id): void => {
      ...
      handle = {
        ...
        listCommands: async (refresh = true) => {
          if (commandClosed) throw new Error(`Engine Suite session is closed: ${String(id)}`)
          if (!refresh && commandCatalog !== undefined) return commandCatalog
          if (commandOperation !== undefined) return commandOperation
        ...
        updateSelection: async (nextSelection, nextApiKey, nextPermissionPreset) => {
          ...
          try {
            ...
            if (nextBridge !== undefined && !bridgeActive) this.childBridge.release(String(id))
            try {
          ...
          if (bridgeActive && nextBridge === undefined) this.childBridge.release(String(id))
  ...
  private observeChildRuntimeEvent(lineage: ParentChildLineageDescriptor, event: AgentStreamEvent): void {
    const current = this.lineageStore.getByChildSessionId(lineage.childSessionId)
    if (current === undefined || current.terminalStatus !== undefined || (current.status !== 'running' && current.status !== 'detached')) return
  ...
  private async attachWorkspace(ownerCtx: Context, cwd: string, sessionId: SessionId): Promise<void> {
    const registry = ownerCtx.get('workspaceRegistry') as WorkspaceRegistryLike | undefined
