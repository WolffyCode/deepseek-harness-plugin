import { normalizeBaseUri, type CreateProviderInput } from './provider/types.js'
...
export function readDebugCodexProviderSeed(
  env: Record<string, string | undefined>,
): DebugCodexProviderSeed | undefined {
  const baseUri = env['DSH_DEBUG_CODEX_BASE_URI']?.trim()
  const apiKey = env['DSH_DEBUG_CODEX_API_KEY']?.trim()
  if (baseUri === undefined && apiKey === undefined) return undefined
