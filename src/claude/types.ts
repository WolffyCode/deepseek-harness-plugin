...
export type ClaudeAdapterEvent =
  | { readonly type: 'session_started'; readonly sessionId: string; readonly catalog: ClaudeCatalog }
  | { readonly type: 'turn_started'; readonly turnId: string; readonly sessionId?: string | undefined }
  | { readonly type: 'timeline'; readonly turnId?: string | undefined; readonly item: ClaudeTimelineItem }
  | { readonly type: 'usage_updated'; readonly turnId?: string | undefined; readonly usage: ClaudeUsage }
  | { readonly type: 'permission_requested'; readonly request: ClaudePermissionRequest }
  | { readonly type: 'permission_resolved'; readonly requestId: string; readonly decision: ClaudePermissionDecision }
  | { readonly type: 'user_question_requested'; readonly request: ClaudeUserQuestionRequest }
  | { readonly type: 'user_question_resolved'; readonly requestId: string; readonly result: ClaudeUserQuestionResult }
  | { readonly type: 'catalog_changed'; readonly catalog: ClaudeCatalog }
  | { readonly type: 'status_changed'; readonly status: string; readonly metadata?: Record<string, unknown> | undefined }
  | { readonly type: 'turn_completed'; readonly turnId?: string | undefined; readonly usage?: ClaudeUsage | undefined; readonly result?: string | undefined }
  | { readonly type: 'turn_failed'; readonly turnId?: string | undefined; readonly error: string }
  | { readonly type: 'turn_canceled'; readonly turnId?: string | undefined }
  | { readonly type: 'process_exited'; readonly error?: string | undefined }
  | { readonly type: 'provider_subagent'; readonly event: SubagentObservation }
...
export const claudeUserAgentDefinitionsBrand: unique symbol = Symbol('claude-user-agent-definitions')
...
export function createClaudeUserAgentDefinitions(definitions: Readonly<Record<string, AgentDefinition>>): ClaudeUserAgentDefinitions {
  return Object.freeze({
    source: 'user' as const,
    definitions: Object.freeze({ ...definitions }),
    [claudeUserAgentDefinitionsBrand]: true as const,
  })
}

export function isClaudeUserAgentDefinitions(value: unknown): value is ClaudeUserAgentDefinitions {
...
export interface ClaudeAgentSession {
  ...
  startTurn(prompt: string, options?: { readonly clientMessageId?: string }): Promise<{ readonly turnId: string }>
  run(prompt: string, options?: { readonly clientMessageId?: string }): Promise<ClaudeRunResult>
  ...
  steer(prompt: string): Promise<{ readonly status: 'accepted' | 'unavailable' }>
