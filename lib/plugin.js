...
export function apply(ctx, config = {}) {
    ...
    const agents = new EngineSuiteAgentService(ctx, suite, credentialRef => resolveApiKey(ctx, credentialRef), catalogReady.promise);
    if (config.primary === true) {
        const registry = ctx.get('agents');
        if (registry === undefined)
            throw new Error('Engine Suite primary mode requires Harness AgentRegistry');
    ...
    registerEngineSuiteSettings(ctx, value => {
        ...
        llmRoutes?.sync();
    }, () => catalogReady.resolve(), debugSettings);
