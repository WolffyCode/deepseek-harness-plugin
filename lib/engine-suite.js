...
function isClaudeGlmModel(model) {
...
function toClaudeModelRecord(providerId, model) {
    const modelId = model.value ?? model.resolvedModel ?? model.id;
    if (!isClaudeGlmModel(model) || modelId.trim().length === 0)
        return undefined;
...
function codexMcpSet(userSet, internalSet) {
    if (internalSet !== undefined)
        materializeInternalMcpServers(internalSet);
...
export function createEngineSuiteRuntime(options = {}) {
    ...
    const runtime = {
        ...
        discoverClaudeModels: async (providerId, options) => {
            const provider = providers.get(providerId);
            if (provider.engineId !== 'claude-cli')
                throw new Error(`provider is not a Claude provider: ${provider.id}`);
            if (options.apiKey.trim().length === 0)
                throw new Error('Claude API key must not be empty');
        ...
        openCodex: async (selection, options) => {
            ...
            const { mcpSet: _mcpSet, skillSet: _skillSet, internalMcpSet: _internalMcpSet, credentialResolver: _credentialResolver, sessionStore: _sessionStore, sessionStoreFlush: _sessionStoreFlush, ...codexOptions } = options;
