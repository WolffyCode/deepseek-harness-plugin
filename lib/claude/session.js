...
export class ClaudeProviderSession {
    ...
    readyError;
    turnStarting = false;
    initializationMessageReceived = false;
    initializationWaiters = new Set();
    _catalog = {
    ...
    async initializeReadiness() {
        try {
            ...
            if (this.closed)
                return;
            if (!this.initializationMessageReceived)
                await this.waitForInitializationMessage();
            if (this.closed)
                return;
            if (this._sessionId === undefined)
                throw new Error('Claude initialization did not provide a session id');
            this.resolveReady();
        }
        catch (error) {
    ...
    buildSdkOptions() {
        ...
        return {
            ...
            ...((this.options.executablePath === undefined && this.options.commandArgs === undefined) ? {} : {
                spawnClaudeCodeProcess: (spawnOptions) => spawn(this.options.executablePath ?? spawnOptions.command, this.options.commandArgs ?? spawnOptions.args, { cwd: spawnOptions.cwd, env: spawnOptions.env, signal: spawnOptions.signal, shell: false, stdio: ['pipe', 'pipe', 'pipe'] }),
            }),
