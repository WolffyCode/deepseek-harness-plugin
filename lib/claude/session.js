...
function contentText(value) {
    ...
    if (record['output_text'] !== undefined)
        return contentText(record['output_text']);
    return serialize(value);
}
function isToolUseBlockType(value) {
...
function permissionDecisionForSdk(decision, originalInput) {
...
function claudeControlPermissionDecision(decision) {
...
export class ClaudeProviderSession {
    ...
    emitToolResult(block, fallbackId) {
    ...
    handleUser(message) {
        ...
        if (Array.isArray(content)) {
            ...
            for (const rawBlock of content) {
                const block = asRecord(rawBlock);
                if (block === undefined || !isToolResultBlockType(block['type']))
                    continue;
                emittedResult = true;
                this.emitToolResult(block);
            }
    ...
    mapModel(value) {
        ...
        const supportedEffortLevels = Array.isArray(item['supportedEffortLevels']) && item['supportedEffortLevels'].every(level => level === 'low' || level === 'medium' || level === 'high' || level === 'xhigh' || level === 'max')
