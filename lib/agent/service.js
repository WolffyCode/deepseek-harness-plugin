...
function deferred() {
    let resolvePromise;
    const promise = new Promise(resolveCurrent => { resolvePromise = resolveCurrent; });
...
export class EngineSuiteAgentService {
    ...
    pendingChildStarts = new Map();
    closingParents = new Set();
    ...
    async delegate(parentSessionId, request) {
        ...
        if (this.closingParents.has(parentSessionId))
            throw new Error(`parent Engine Suite session is closing: ${parentSessionId}`);
        ...
        const pending = this.pendingChildStarts.get(parentSessionId) ?? new Set();
        ...
        try {
            ...
            if (pendingStarts !== undefined && pendingStarts.size === 0)
                this.pendingChildStarts.delete(parentSessionId);
            ...
            if (remaining > 0)
                ...
                if ((this.children.get(parentSessionId)?.size ?? 0) === 0)
    ...
    async resume(ownerCtx, options) {
        ...
        return this.withSessionOperation(id, async () => {
            ...
            if (lineage?.status === 'starting')
                throw new Error(`cannot resume child session while it is starting: ${id}`);
    ...
    async archiveChild(childSessionId) {
        ...
        return this.withSessionOperation(id, async () => {
            ...
            if (lineage.status === 'archived')
                return lineage;
            if (lineage.status === 'starting')
                throw new Error(`cannot archive child session while it is starting: ${id}`);
    ...
    async detachChild(childSessionId) {
        ...
        return this.withSessionOperation(id, async () => {
            ...
            if (lineage.status === 'starting')
                throw new Error(`cannot detach child session while it is starting: ${id}`);
            if (lineage.status === 'detached')
                return lineage;
            const handle = this.findLiveSession(id);
    ...
    async cancelChild(childSessionId) {
        ...
        return this.withSessionOperation(id, async () => {
            ...
            if (lineage === undefined)
                throw new Error(`unknown child Engine Suite session: ${id}`);
            if (lineage.status === 'starting')
                throw new Error(`cannot cancel child session while it is starting: ${id}`);
    ...
    async cancelChildren(parentSessionId) {
        ...
        await Promise.all(children.map(async (child) => {
            ...
            if (this.shouldDisposeWithParent(child))
                await child.dispose().catch(() => undefined);
        }));
    }
    async waitForChildStarts(parentSessionId) {
        while (true) {
    ...
    async delegateNative(parent, request, selection, depth, currentChildren, lineage, onPublished) {
        ...
        try {
            ...
            if (bindingPublished) {
                if (previousBinding === undefined)
                    await this.bindings.remove(String(id)).catch(() => { });
                else
                    await this.bindings.put(previousBinding).catch(() => { });
            }
            throw error;
        }
    }
