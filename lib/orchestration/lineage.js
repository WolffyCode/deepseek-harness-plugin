...
function validateDescriptor(input) {
    ...
    if (input.status !== 'starting' && input.status !== 'running' && input.status !== 'completed' && input.status !== 'failed' && input.status !== 'canceled' && input.status !== 'archived' && input.status !== 'detached')
...
function transitionDescriptor(current, type, timestamp) {
    ...
    if (type === 'resume') {
        if (current.status === 'starting')
            throw new Error('cannot resume lineage while it is starting');
    ...
    if (type === 'detach') {
        if (current.status === 'starting')
            throw new Error('cannot detach lineage while it is starting');
        if (current.status === 'archived')
    ...
    if (type === 'archive') {
        if (current.status === 'starting')
            throw new Error('cannot archive lineage while it is starting');
        if (current.status === 'archived')
    ...
    if (current.status !== 'starting' && current.status !== 'running' && current.status !== 'detached')
        throw new Error(`cannot finish lineage in ${current.status} state`);
    if (type === 'result' && current.status === 'starting')
        throw new Error('cannot complete lineage while it is starting');
