    ...
    async handle(request, response) {
        ...
        try {
            ...
            for (const key of Object.keys(value))
                if (!allowed.has(key))
                    throw new Error(`unsupported child bridge field: ${key}`);
            const parentSessionId = textField(value['parentSessionId'], 'parentSessionId');
            if (!this.parents.has(parentSessionId))
                throw new Error('parentSessionId is not authorized for this bridge');
            const parsed = {
                parentSessionId,
