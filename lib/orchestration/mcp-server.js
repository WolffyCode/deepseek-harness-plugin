...
for (const key of Object.keys(process.env)) {
    if (key === 'DSH_ENGINE_SUITE_BRIDGE_TOKEN' || key === 'DSH_ENGINE_SUITE_BRIDGE_URL' || key === 'DSH_ENGINE_SUITE_PARENT_SESSION')
        continue;
...
function localBridgeUrl(value) {
    if (value === undefined)
        throw new Error('Engine Suite child bridge URL is missing');
    let url;
    try {
        url = new URL(value);
    }
    catch {
        throw new Error('Engine Suite child bridge URL is invalid');
    }
...
async function delegate(value) {
    ...
    try {
        ...
        throw new Error(`child bridge returned HTTP ${response.status}`);
    }
    if (!response.ok || payload.ok !== true || payload.value === undefined)
        throw new Error(payload.error ?? `child bridge returned HTTP ${response.status}`);
    const result = {
...
rl.on('line', line => {
    void (async () => {
        ...
        try {
            ...
            send(undefined, undefined, error);
            return;
        }
        ...
        if (method === 'tools/call') {
            const params = typeof request.params === 'object' && request.params !== null && !Array.isArray(request.params)
                ? request.params
                : {};
            if (params['name'] !== 'engine_suite_delegate')
