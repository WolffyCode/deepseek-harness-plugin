...
function assertAllowedKeys(value, allowedKeys, optionalKeys, label) {
...
function strictObjectSchema(schema, allowedKeys, label) {
    return z.transform(schema, (value) => {
        if (typeof value !== 'object' || value === null || Array.isArray(value)) {
            throw new Error(`${label} must be an object`);
        }
...
function normalizeStdioMcpServer(value) {
    assertAllowedKeys(value, ['id', 'name', 'transport', 'command', 'args', 'environment', 'credentialRefs'], ['args', 'environment', 'credentialRefs'], 'stdio MCP server');
...
function normalizeHttpMcpServer(value) {
    assertAllowedKeys(value, ['id', 'name', 'transport', 'url', 'headers', 'credentialRefs'], ['headers', 'credentialRefs'], 'http MCP server');
...
function normalizeSseMcpServer(value) {
    assertAllowedKeys(value, ['id', 'name', 'transport', 'url', 'headers', 'credentialRefs'], ['headers', 'credentialRefs'], 'sse MCP server');
    const headers = optionalStringRecord(value['headers'], 'MCP headers');
    const credentialRefs = optionalStringRecord(value['credentialRefs'], 'MCP credentialRefs');
    return {
        id: requiredString(value['id'], 'MCP server id'),
...
export const EngineSuiteSettingsSchema = strictObjectSchema(z.object({
    ...
    suite.assets.replaceMcpSets((value.mcpSets ?? []).map(set => ({
        ...
        servers: set.servers.map(server => {
            switch (server.transport) {
                case 'stdio': {
                    assertAllowedKeys(server, ['id', 'name', 'transport', 'command', 'args', 'environment', 'credentialRefs'], ['args', 'environment', 'credentialRefs'], 'stdio MCP server');
                ...
                case 'http': {
                    assertAllowedKeys(server, ['id', 'name', 'transport', 'url', 'headers', 'credentialRefs'], ['headers', 'credentialRefs'], 'http MCP server');
                ...
                case 'sse': {
                    assertAllowedKeys(server, ['id', 'name', 'transport', 'url', 'headers', 'credentialRefs'], ['headers', 'credentialRefs'], 'sse MCP server');
                    const headers = optionalStringRecord(server.headers, 'MCP headers');
                    const credentialRefs = optionalStringRecord(server.credentialRefs, 'MCP credentialRefs');
                    return {
                        id: requiredString(server.id, 'MCP server id'),
